...
public:
    ...
    void show_film_detail(Customer* customer,
    std::vector<std::string>seperated_input, ProgramData* program_data);
    void print_film_detail(std::vector<Films*>films, std::string film_id, int film_code);
    void print_replie(std::vector<Customer*>customers_in_system,
    std::vector<std::vector<std::string> >comments_of_film, int i, int &conter);

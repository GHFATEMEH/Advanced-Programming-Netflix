...
bool ErrorCheakingRepliesAndDeleteComments::cheak_commant_id(string commant_id, vector<Films*>films,
int i) {
...
Publisher* publisher, string commant_id) {
    ...
    for(int i = 0; i < films.size(); i++) {
        if( (films[i]->get_id() == id) && (cheak_commant_id(commant_id, films, i)));
            return;
    }

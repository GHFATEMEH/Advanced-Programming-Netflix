...
void HandlingOfCommandReplies::add_notification_replie(Publisher* publisher, string film_id,
...
void HandlingOfCommandReplies::input_replies(vector<string>separated_input,
ProgramData* program_data) {
    Publisher* publisher = program_data->find_publisher_in_system();
    string film_id = separated_input[4];
    string commant_id = separated_input[6];
    ...
    error_cheaking_replies.cheak_film_id_and_comment_is_for_publisher(film_id, publisher, commant_id);
    add_notification_replie(publisher, film_id, program_data, commant_id, content);
    cout << OK << "\n";
}

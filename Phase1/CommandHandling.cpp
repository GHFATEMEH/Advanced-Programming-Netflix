...
bool CommandHandling::cheak_first_part_of_input(vector<string>separated_input) {
    ...
    || (separated_input[0] == PUT ) || (separated_input[0] == DELETE))
        return true;
    else 
...
void CommandHandling::cheak_input_for_each_command(vector<string>separated_input) {
    if(separated_input[1] == SIGNUP) {
        ...
        command_login.cheak_input_for_login(separated_input, program_data);
    ...
    else  if(separated_input[1] == FILMS) {
            ...
            HandlingOfCommandReplies command_replies;
            command_replies.input_replies(separated_input, program_data);
    }
    else  if(separated_input[1] == COMMENTS) {
    ...
    //else  if(separated_input_by_delimiters[1] == NOTIFICATIONS)
}

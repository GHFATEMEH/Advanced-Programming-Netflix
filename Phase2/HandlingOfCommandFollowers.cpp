...
void HandlingOfCommandFollowers::save_publisher_followers_for_publisher(int publisher_id,
Publisher* publisher, ProgramData* program_data) {
    string username = publisher->get_username();
    string email = publisher->get_email();
    int publisher_id_to_save = publisher->get_id();
    vector<Publisher*>publisher_followe =  program_data->get_publisher();
    for(int i = 0; i < publisher_followe.size(); i++) {
        if(publisher_followe[i]->get_id() == publisher_id){
            publisher_followe[i]->add_followers(publisher_id_to_save, username, email);
            publisher_followe[i]->notification_you_followed(username,publisher_id_to_save);
            break;
        }
    }
}
...
Publisher* publisher, ProgramData* program_data) {
    vector<int>list_of_publisher_followed;
    if(program_data->get_type_of_user_in_system() == CUSTOMER)
        list_of_publisher_followed = customer->get_list_of_publisher_followed();
    else
        list_of_publisher_followed = publisher->get_list_of_publisher_followed();
    int id = stoi(publisher_id);
...
ProgramData* program_data, Customer* customer, Publisher* publisher) {
    ...
    if(did_not_followe_before(publisher_id, customer, publisher, program_data)) {
        if(program_data->get_type_of_user_in_system() == CUSTOMER) {
            customer->followe_publisher(stoi(publisher_id));
            save_publisher_followers(stoi(publisher_id), customer, program_data);
        }
        else {
            publisher->followe_publisher(stoi(publisher_id));
            save_publisher_followers_for_publisher(stoi(publisher_id), publisher, program_data);
        }
    }
}
...
ProgramData* program_data) {
    ...
    Customer* customer = program_data->find_customer_in_system();
        if(separated_input[0] == POST) {
            followe_publisher(separated_input, program_data, customer, publisher);
            cout << OK << "\n";
        }

...
Films::Films(int _id, string _year, string _length, string _price,
    string _name, string _summary, string _director, string _exist_or_delete) {
    id = _id;
    year = _year;
    length = _length;
    price = _price;
...
void Films::set_summary(string _summary) {
    summary = _summary;
}
...
void Films::set_customers_rate_to_film(int rate, int customer_id) {
    for(int i = 0; i < customers_rate_to_film.size(); i++) {
        if(customers_rate_to_film[i][0] == customer_id) {
            customers_rate_to_film[i][1] = rate;
            return;
        }
    }
    customers_rate_to_film.push_back(vector<int>());
    int size_of_row = customers_rate_to_film.size();
    customers_rate_to_film[size_of_row - 1].push_back(customer_id);
    customers_rate_to_film[size_of_row - 1].push_back(rate);
}

void Films::set_rate() {
    int number_of_rate = customers_rate_to_film.size();
    int sum_of_rate = 0;
    for(int i = 0; i < customers_rate_to_film.size(); i++){
        sum_of_rate += customers_rate_to_film[i][1];
    rate = (sum_of_rate/number_of_rate);
    }
}
